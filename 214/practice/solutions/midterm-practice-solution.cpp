...
void swapByRef(char c1, char c2) {
  char temp = c1;
  c1 = c2;
  c2 = temp;
}
...
void swapByAddress(char &c1, char &c2) {
  char temp = c1;
  c1 = c2;
  c2 = temp;
}
...
struct ArrayStats {
  ...
  ArrayStats(int min, int max) : minIndex(min), maxIndex(max) {};
};

ArrayStats getArrayStats(const std::array<int, 6> &stats) {
  auto max = stats[0];
  auto min = stats[0];
  int mindex = 0;
  int maxdex = 0;
  ...
  return ArrayStats(mindex, maxdex);
}
...
void testQ5() {
  ...
  std::cout << "min:" << stats.minIndex << ", max:" << stats.maxIndex
            << '\n'; //
  // output: min: 2, max: 4
}
...
void censorString(char *arr, const std::vector<char> &bad) {
...
//		  Initialize each Point x and y to a random int in the range
//[0...param2] 		  (inclusive - ie: [0...2] includes 0, 1, and 2)
//
...
//			  ...
//			  Point[x,y]
// - [1 pt] use a ranged for loop and make it as efficient as possible (use a
// reference)
//
...
std::vector<Point> createRandomPoints(int num, int max) {
  Point points;
  std::vector<Point> p;
...
void testQ7() {
  ...
  int max = 3;
  int num = 4;
  auto results = createRandomPoints(num, max);
  print(results);
}
...
// =============================================================
// Question [4 pts]
// - Uncomment and complete the function that has beens started:
// concatCStrings()
...
// - Ensure the new string can be printed properly (without errors or extra
// chars)
//
...
char *concatCStrings(const char msg1[], const char msg2[]) {
  ...
  const int len = len2 + len1 + 1;
  char *newMsg = new char[len];

  strcpy(newMsg, msg1);
  strcat(newMsg, msg2);
  return newMsg; // replace this return value as part of this question
}
...
//
// STEP 1: Write a function getGradesFromUser() that returns a vector of
// doubles.
//         It should:
...
void testQ9() {
  std::cout << "testQ9()----------\n";
  auto grades = getGradesFromUser();
  auto avg = getAvgGrade(grades);
  std::cout << avg << "\n";
}
...
//   and returns by value.  If a change is made to the value returned (in the
//   caller), it doesn't (currently) affect the original.
//
...
// Hint: Changes are required in both functions (getLongerString() and
...
Employee *findEmployeeById(Employee *pEmployee, int id) {
  Employee *pCurrent = pEmployee;
