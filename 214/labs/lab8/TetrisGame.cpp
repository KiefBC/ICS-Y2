...
TetrisGame::TetrisGame(sf::RenderWindow &window, sf::Sprite &blockSprite,
                       const Point &gameboardOffset,
                       const Point &nextShapeOffset)
    : window(window), blockSprite(blockSprite),
      gameboardOffset(gameboardOffset), nextShapeOffset(nextShapeOffset),
      score(0), secondsSinceLastTick(0.0), shapePlacedSinceLastGameLoop(false),
  ...
  if (!backgroundMusic.openFromFile("audio/Tetromino-Flow.ogg")) {
    ...
    backgroundMusic.setVolume(
        VOLUME_INITIAL); // Set volume to 50% (0-100 range)
...
void TetrisGame::draw() {
  drawGameboard();
  // drawTetromino(ghostShape, gameboardOffset, 0.3f);  // Draw ghost shape with
  // 30% opacity
...
/// @brief Moves the current shape down one line if possible, otherwise locks
/// it.
...
/// @brief Sets the current shape to the next shape and sets its grid location
...
/// @brief Locks the shape on the board by setting the content of the shape's
/// block locations to the shape's color.
...
void TetrisGame::drawBlock(const Point &topLeft, int xOffset, int yOffset,
                           TetColor color, float alpha) {
  blockSprite.setTextureRect(sf::IntRect(static_cast<int>(color) * BLOCK_WIDTH,
  ...
  // blockSprite.setColor(sf::Color(255, 255, 255, static_cast<sf::Uint8>(alpha
  // * 255)));
  window.draw(blockSprite);
  blockSprite.setColor(sf::Color(255, 255, 255, 255));
}

/// @brief Draws the gameboard by iterating through each cell and drawing a
/// block if it is not empty.
...
void TetrisGame::drawTetromino(const GridTetromino &tetromino,
                               const Point &topLeft, float alpha) {
...
/// @brief Updates the score display by setting the string of the score text to
/// the current score.
...
bool TetrisGame::isWithinBorders(const GridTetromino &shape) const {
  ...
  for (const Point &loc : mappedLocs) {
    ...
    if (loc.getX() < 0 || loc.getX() >= Gameboard::MAX_X || loc.getY() < 0 ||
...
/// @brief Determines the seconds per tick based on the score by dividing the
/// maximum seconds per tick by the score divided by 100 plus 1.
void TetrisGame::determineSecondsPerTick() {
  secondsPerTick = MAX_SECONDS_PER_TICK / (score / 100 + 1);
}

// MY OWN METHODS CUS IM COOL AS F
...
/// @brief Updates the ghost shape that is the current shape dropped to the
/// bottom.
