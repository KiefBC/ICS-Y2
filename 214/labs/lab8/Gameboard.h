...
private:
  ...
  const Point spawnLoc{(MAX_X / 2) - 1, 0}; // Added -1 to spawnLoc.x to center the tetromino
...
public:
  ...
  void setContent(const Point&, int);
  ...
  void setContent(const std::vector<Point>&, int);
  ...
  // otherwise
  bool areAllLocsEmpty(std::vector<Point>&) const;
...
private: // This is commented out to allow us to test.
  ...
  bool isValidPoint(const Point&) const;
  ...
  bool isValidPoint(int, int) const;
  ...
  // - return: nothing
  void removeRows(std::vector<int>&);
};
