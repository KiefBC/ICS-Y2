...
void TestSuite::runTestSuite() {
  std::cout << "=== Running TestSuite ========================="
            << "\n"; // run some sanity tests on our classes to ensure they're
                     // working as expected.
  testGameboardClass();
...
bool TestSuite::isGameboardEmpty(const Gameboard &g) {
  for (int x = 0; x < Gameboard::MAX_X; x++) {
    for (int y = 0; y < Gameboard::MAX_Y; y++) {
      if (g.getContent(x, y) != Gameboard::EMPTY_BLOCK) {
        return false;
      }
    }
  }
...
void TestSuite::testGameboardClass() {
  ...
  for (int x = 0; x < Gameboard::MAX_X; x++) {
    for (int y = 0; y < Gameboard::MAX_Y; y++) {
      assert(g.getContent(x, y) == Gameboard::EMPTY_BLOCK &&
             "Gameboard was not initialized with empty blocks");
    }
  }
  ...
  assert(g.getContent(Gameboard::MAX_X - 1, Gameboard::MAX_Y - 1) == 2 &&
         "Gameboard.setContent() - unexpected result"); // was grid content set?
  g.setContent(Gameboard::MAX_X - 1, Gameboard::MAX_Y - 1,
               Gameboard::EMPTY_BLOCK);
  assert(g.getContent(Gameboard::MAX_X - 1, Gameboard::MAX_Y - 1) ==
             Gameboard::EMPTY_BLOCK &&
         "Gameboard.setContent() - unexpected result"); // was grid content set?
  ...
  g.fillRow(0, 1);
  assert(g.getContent(0, 0) == 1 &&
         "Gameboard.fillRow() - row should be filled with 1, but first element "
         "is not 1"); // is the first spot in the row what we expect?
  assert(g.getContent(Gameboard::MAX_X - 1, 0) == 1 &&
         "Gameboard.fillRow() - row should be filled, but last element is "
         "still 1"); // is the last spot in the row what we expect?
  assert(g.isRowCompleted(0) == true &&
         "Gameboard.fillRow() - row should be filled, but does not appear to "
         "be completed"); // was row 0 completed?

  // test isRowCompleted()
  g.setContent(0, 0, Gameboard::EMPTY_BLOCK);
  assert(
      g.isRowCompleted(0) == false &&
      "Gameboard.isRowCompleted() clearing a single entry should make row "
      "incomplete"); // did a single incomplete entry make the row incomplete?

  // test clearRow()
  g.fillRow(0, 1);
  assert(g.isRowCompleted(0) == true &&
         "Gameboard.fillRow() - row should be filled with 1, but does not "
         "appear to be completed"); // ensure row filled
  g.fillRow(0, Gameboard::EMPTY_BLOCK);
  assert(g.isRowCompleted(0) == false &&
         "Gameboard.fillRow() - row should be empty, but appears to be "
         "completed"); // did row 0 get cleared

  // test isEmpty
  g.empty();
  assert(TestSuite::isGameboardEmpty(g) == true &&
         "Gameboard.isGameboardEmpty() returned false when true expected");
  g.setContent(0, 0, 1);
  assert(TestSuite::isGameboardEmpty(g) == false &&
         "Gameboard.isGameboardEmpty() returned true when false expected");
  g.empty();
  assert(TestSuite::isGameboardEmpty(g) == true &&
         "Gameboard.isGameboardEmpty() returned false when true expected");
  ...
  assert(TestSuite::isGameboardEmpty(g) == true &&
         "Gameboard.isGameboardEmpty() "
         "returned false when true "
         "expected"); // verify the board
                      // is empty
  ...
  for (int x = 0; x < Gameboard::MAX_X; x++) {
    assert(g.getContent(x, 1) == g.getContent(x, 3) &&
           "Gameboard.copyRowIntoRow()"
           ", rows expected to match "
           "but do not"); // compare
                          // row 1 with
                          // row 3,
                          // does it
  ...
  assert(g.getContent(0, 0) == Gameboard::EMPTY_BLOCK &&
         "Gameboard.removeRow() expected row to be empty, but it is not");
  ...
  assert(g.getContent(0, 0) == Gameboard::EMPTY_BLOCK &&
         "Gameboard.removeRows() seems to have failed");
  ...
  assert(completedRows.size() == 2 && completedRows[0] == 2 &&
         completedRows[1] == 4 &&
  ...
  assert(g.removeCompletedRows() == 0 &&
         "Gameboard.removeCompletedRows() should return 0");
  g.fillRow(1, 2);
  g.fillRow(3, 2);
  assert(g.removeCompletedRows() == 2 &&
         "Gameboard.removeCompletedRows() should return 2");
  assert(TestSuite::isGameboardEmpty(g) == true &&
         "Gameboard.isGameboardEmpty() should return true");
  ...
  assert(g.getContent(1, 0) == Gameboard::EMPTY_BLOCK &&
         "Gameboard.removeCompletedRows() first row should be empty");
  assert(g.getContent(1, 1) == 0 &&
         "Gameboard.removeCompletedRows() unexpected results"); // row 0 copied
                                                                // into row 1
  assert(g.getContent(1, 2) == 1 &&
         "Gameboard.removeCompletedRows() unexpected results"); // row 1 copied
                                                                // into row 2
  assert(g.getContent(1, 3) == 2 &&
         "Gameboard.removeCompletedRows() unexpected results"); // row 2 copied
                                                                // into row 3
  assert(g.getContent(1, 4) == Gameboard::EMPTY_BLOCK &&
         "Gameboard.removeCompletedRows() unexpected results"); // row 4 is
                                                                // still empty
  ...
  assert(
      g.areAllLocsEmpty(testPoints) == true &&
      "Gameboard.areAllLocsEmpty() expected true but was false"); // should
                                                                  // return true
                                                                  // since all
                                                                  // points are
                                                                  // empty
  ...
  assert(
      g.areAllLocsEmpty(testPoints) == false &&
      "Gameboard.areAllLocsEmpty() expected false but was true"); // should
                                                                  // return
                                                                  // false since
                                                                  // 2,2
                                                                  // contains
  ...
  g.empty();
  std::vector<Point> invalidPoints = {Point(-3, -20), Point(200, 23),
                                      Point(-30, 150)};
  assert(g.areAllLocsEmpty(invalidPoints) == true &&
         "Gameboard.areAllLocsEmpty() expected true but was false");

  std::vector<Point> mixedPoints1 = {Point(-3, -20), Point(200, 23),
                                     Point(6, 6), Point(5, 5)};
  assert(g.areAllLocsEmpty(mixedPoints1) == true &&
         "Gameboard.areAllLocsEmpty() expected true but was false");

  g.fillRow(3, 3);
  std::vector<Point> mixedPoints2 = {Point(-3, -20), Point(200, 23),
                                     Point(6, 6), Point(3, 3)};
  assert(g.areAllLocsEmpty(mixedPoints2) == false &&
         "Gameboard.areAllLocsEmpty() expected false but was true");
  ...
  // These lines will cause compile time errors if you have methods in your
  // Gameboard class that should be labelled as const (because the methods don't
  // change the internal member variables of yoru class). (see LearnCpp 13.2-
