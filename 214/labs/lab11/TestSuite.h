...
// of the MyVector class.  To enable testing add the following line to your
// main().
//     assert(TestSuiteTemplate::runTestSuite());
// You will need to uncomment the #define statements (above) when you have
// finished each stage.
class TestSuite {
public:
  ...
  static bool testStage1() {
    ...
    // create an instance in a block, when the block goes out of scope vEmpty
    // will be destroyed via the destructor.  Ensure the destructor works
    // correctly Ensure the constructor didn't make an allocation call if size
    // was 0.
    {
      MyVector vEmpty;
    }
  ...
  static bool testStage3() {
    ...
    // this requires a secondary override to the [] operator labelled as const
    // in 2 places
  ...
  static bool testStage4() {
    ...
    // even if we reserve, we should be able to access the memory until we
    // resize the vector
    try {
      v1[0] = 0;
      assert(false && "expected an exception to be thrown here!");
