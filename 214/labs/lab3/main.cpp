...
int main() {
  ...
  const double averageCardsDrawn =
      static_cast<double>(totalCardsDrawn) / totalTrials;
  ...
  auto duration1 = std::chrono::duration_cast<std::chrono::milliseconds>(
      endTime1 - startTime);
  std::cout << "Time taken for simulation: " << duration1.count() / 60000
            << " minutes " << (duration1.count() % 60000) / 1000 << " seconds"
            << std::endl;
  ...
  const double averageCardsDrawn2 =
      static_cast<double>(totalCardsDrawn2) / totalTrials2;
  ...
  auto duration2 = std::chrono::duration_cast<std::chrono::milliseconds>(
      endTime2 - startTime2);
  std::cout << "Time taken for simulation: " << duration2.count() / 60000
            << " minutes " << (duration2.count() % 60000) / 1000 << " seconds"
            << std::endl;
}
